...
export function buildArchive(input: BuildArchiveInput): Uint8Array {
  const { fileName, doc, originalName, originalDoc, charts = [], generatedAt = new Date(), lang = 'ru' } = input;
  const en = lang === 'en';
  const diff = diffDocs(originalDoc, doc, lang);

  const files: Record<string, Uint8Array> = {
    [safeEntryName(fileName)]: iesBytes(doc),
    [en ? 'File check.txt' : 'Проверка файла.txt']: strToU8(formatAuditReport(fileName, auditDoc(doc, lang), lang)),
    [en ? 'Processing report.txt' : 'Отчёт об обработке.txt']: strToU8(formatProcessingReport(originalName, fileName, diff, generatedAt, lang)),
    [en ? 'Metrics.csv' : 'Показатели.csv']: strToU8(metricsCsv(doc, lang)),
  };

  // Одинаковые имена молча затирали бы друг друга в архиве — разводим их.
  const usedLabels = new Set<string>();
  charts.forEach((chart, i) => {
    let label = safeEntryName(chart.label) || `${en ? 'Chart' : 'Диаграмма'} ${i + 1}`;
    ...
    const dir = en ? 'Charts' : 'Диаграммы';
    files[`${dir}/${label}.svg`] = strToU8(chart.svg);
    if (chart.png) files[`${dir}/${label}.png`] = chart.png;
  });

  // Исходный файл кладём только если он реально отличается — иначе в архиве
  // просто лежала бы вторая копия того же самого.
  if (diff.length > 0) {
    files[`${en ? 'Source file' : 'Исходный файл'}/${safeEntryName(originalName)}`] = iesBytes(originalDoc);
  }
